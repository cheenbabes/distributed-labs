"""
API Service - Middle layer with limited concurrency (30 concurrent requests).
...
DOWNSTREAM_URL = os.getenv("DOWNSTREAM_URL", "http://orders:8002")
LATENCY_MIN_MS = int(os.getenv("LATENCY_MIN_MS", "10"))
LATENCY_MAX_MS = int(os.getenv("LATENCY_MAX_MS", "25"))
...
async def process(request: Request):
    """Process request - validates and forwards to orders service."""
    ...
    try:
        ...
        try:
            # Simulate API processing time (validation, auth, etc.)
            latency_ms = random.randint(LATENCY_MIN_MS, LATENCY_MAX_MS)
            await asyncio.sleep(latency_ms / 1000.0)
            current_span.set_attribute("api_latency_ms", latency_ms)
            ...
            async with httpx.AsyncClient(timeout=TIMEOUT_SECONDS) as client:
                try:
                    ...
                    TIMEOUTS_TOTAL.labels(service=SERVICE_NAME, downstream="orders").inc()
                    REQUEST_COUNT.labels(
                        service=SERVICE_NAME,
                        method="GET",
                        endpoint="/process",
