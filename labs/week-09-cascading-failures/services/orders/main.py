"""
Orders Service - Order processing with limited concurrency (20 concurrent requests).
...
# Configuration
SERVICE_NAME = os.getenv("OTEL_SERVICE_NAME", "orders")
OTEL_ENDPOINT = os.getenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://otel-collector:4317")
DOWNSTREAM_URL = os.getenv("DOWNSTREAM_URL", "http://inventory:8003")
LATENCY_MIN_MS = int(os.getenv("LATENCY_MIN_MS", "15"))
LATENCY_MAX_MS = int(os.getenv("LATENCY_MAX_MS", "35"))
MAX_CONCURRENT = int(os.getenv("MAX_CONCURRENT", "20"))
TIMEOUT_SECONDS = float(os.getenv("TIMEOUT_SECONDS", "6.0"))
...
async def process(request: Request):
    """Process order - checks inventory and creates order."""
    ...
    try:
        ...
        try:
            ...
            current_span.set_attribute("orders_latency_ms", latency_ms)

            # Call inventory service with timeout
            async with httpx.AsyncClient(timeout=TIMEOUT_SECONDS) as client:
                try:
                    response = await client.get(f"{DOWNSTREAM_URL}/check")
                    response.raise_for_status()
                    downstream_result = response.json()
                except httpx.TimeoutException:
                    TIMEOUTS_TOTAL.labels(service=SERVICE_NAME, downstream="inventory").inc()
                    REQUEST_COUNT.labels(
                        service=SERVICE_NAME,
                        method="GET",
                        endpoint="/process",
